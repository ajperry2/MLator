...
def match_bipartite_greedy(weight_matrix):
    ...
    Arguments:
        ...
        matrix
            for the matching process. If `(m,n)` is the shape of the weight
            matrix,
            it must be `m <= n`. The weights can be integers or floating point
            numbers. The matching process will maximize, i.e. larger weights
            are
    ...
    for _ in range(num_ground_truth_boxes):
        ...
        anchor_indices = np.argmax(weight_matrix,
                                   axis=1)  # Reduce along the anchor box axis.
        ...
        # matched
        # anchor box to all zeros. This ensures that those boxes will not be
        #  matched again,
...
def match_multi(weight_matrix, threshold):
    ...
    matches along the first axis subject to the constraint that the weight
    of a match
    must be greater than or equal to `threshold` in order to produce a match.

    If the weight matrix contains elements that should be ignored, the row
    or column
    representing the respective elemet should be set to a value below
    `threshold`.
    ...
    Returns:
        Two 1D Numpy arrays of equal length that represent the matched
        indices. The first
        array contains the indices along the first axis of `weight_matrix`,
        the second array
    ...
    ground_truth_indices = np.argmax(weight_matrix,
                                     axis=0)  # Array of shape (
    # weight_matrix.shape[1],)
    overlaps = weight_matrix[
        ground_truth_indices, all_anchor_indices]  # Array of shape (
    # weight_matrix.shape[1],)
